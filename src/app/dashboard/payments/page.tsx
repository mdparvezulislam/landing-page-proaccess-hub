    ...
    <div className="max-w-5xl mx-auto space-y-3 lg:space-y-8 pb-8 lg:pb-20">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="p-3 lg:p-5 rounded-[16px] lg:rounded-[24px] bg-info/5 border border-info/10">
          ...
          <p className="text-sm lg:text-2xl font-black text-white">{nextDueDate?.toLocaleDateString() || 'N/A'}</p>
          <p className={`text-[10px] lg:text-sm font-bold mt-0.5 lg:mt-1 ${daysUntilDue <= 3 ? 'text-red-500' : daysUntilDue <= 7 ? 'text-amber-500' : 'text-success'}`}>
        ...
        <div className="p-3 lg:p-5 rounded-[16px] lg:rounded-[24px] bg-white/[0.02] border border-white/5">
          ...
          <p className="text-lg lg:text-3xl font-black text-success">{stats.progress}%</p>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        ...
        <h3 className="text-[11px] lg:text-sm font-black uppercase tracking-widest text-text-muted mb-3 lg:mb-4 flex items-center gap-1.5 lg:gap-2">
          <Send className="w-3.5 h-3.5 lg:w-4 lg:h-4 text-amber-500" /> Send Payment To
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 lg:gap-3">
          {paymentMethods.map((method: any) => (
            <div key={method.id} className="p-3 lg:p-4 rounded-xl lg:rounded-2xl bg-white/[0.02] border border-white/5">
              <p className="text-[10px] lg:text-xs font-black uppercase tracking-widest text-amber-500 mb-1.5 lg:mb-2">{method.name}</p>
              {method.accountHolder && (
                <p className="text-[9px] lg:text-[10px] text-text-muted mb-0.5 lg:mb-1">A/C: {method.accountHolder}</p>
              )}
              <div className="flex items-center justify-between">
                <span className="font-mono text-[11px] lg:text-sm text-white truncate mr-1">{method.number}</span>
                <button onClick={() => copyToClipboard(method.id, method.number)}
                  className="w-7 h-7 lg:w-8 lg:h-8 rounded-lg bg-white/5 flex items-center justify-center hover:bg-white/10 transition-all flex-shrink-0">
                  {copiedId === method.id ? <Check className="w-3 h-3 lg:w-3.5 lg:h-3.5 text-success" /> : <Copy className="w-3 h-3 lg:w-3.5 lg:h-3.5 text-text-muted" />}
                </button>
              </div>
            </div>
          ))}
      ...
      <motion.form onSubmit={handleSubmit} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        className="p-4 lg:p-8 rounded-[20px] lg:rounded-[32px] bg-white/[0.02] border border-white/5 space-y-4 lg:space-y-6">
        <h3 className="text-sm lg:text-lg font-black tracking-tight flex items-center gap-1.5 lg:gap-2">
          <DollarSign className="w-4 h-4 lg:w-5 lg:h-5 text-amber-500" /> Submit Payment Proof
        </h3>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 lg:gap-4">
          <div className="space-y-1.5 lg:space-y-2">
            <label className="text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-text-muted">Amount (BDT) *</label>
            <input type="number" value={amountBDT || ''} onChange={(e) => {
              const val = Number(e.target.value);
              setAmountBDT(val);
              setAmountUSDT(Math.round(val / 125));
            }}
              className="w-full bg-white/5 border border-white/10 rounded-xl lg:rounded-2xl p-3 lg:p-4 text-white text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50" />
          </div>
          <div className="space-y-1.5 lg:space-y-2">
            <label className="text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-text-muted">Amount (USDT)</label>
        ...
        <div className="space-y-1.5 lg:space-y-2">
          <label className="text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-text-muted">Transaction ID *</label>
          <input type="text" value={transactionId} onChange={(e) => setTransactionId(e.target.value)} placeholder="Enter your transaction/TrxID"
            className="w-full bg-white/5 border border-white/10 rounded-xl lg:rounded-2xl p-3 lg:p-4 text-white text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50" />
        </div>

        <div className="space-y-1.5 lg:space-y-2">
          <label className="text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-text-muted">Payment Method</label>
          <div className="flex flex-wrap gap-2 lg:gap-3">
            {paymentMethods.map((method: any) => (
              <button key={method.id} type="button" onClick={() => setPaymentMethod(method.id)}
                className={`px-4 lg:px-6 py-2 lg:py-3 rounded-xl lg:rounded-2xl text-[10px] lg:text-xs font-black uppercase tracking-widest transition-all border ${paymentMethod === method.id ? 'bg-amber-500/10 border-amber-500/30 text-amber-500' : 'bg-white/5 border-white/10 text-text-muted hover:text-white'}`}>
                {method.name}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-1.5 lg:space-y-2">
          <label className="text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-text-muted">Note (Optional)</label>
          <textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Any additional information..."
            className="w-full bg-white/5 border border-white/10 rounded-xl lg:rounded-2xl p-3 lg:p-4 text-white text-sm min-h-[64px] lg:min-h-[80px] focus:outline-none focus:ring-2 focus:ring-amber-500/50" />
        </div>

        <div className="p-3 lg:p-4 rounded-xl lg:rounded-2xl bg-info/10 border border-info/20 flex items-start gap-2 lg:gap-3">
          <Globe className="w-4 h-4 lg:w-5 lg:h-5 text-info flex-shrink-0 mt-0.5" />
          <p className="text-info text-[10px] lg:text-xs font-medium">After submitting, admin will verify your payment. You will receive a notification once approved.</p>
        </div>

        <button type="submit" disabled={submitPayment.isPending || !amountBDT || !transactionId}
          className="w-full py-3.5 lg:py-5 rounded-xl lg:rounded-2xl bg-gradient-to-r from-amber-500 to-orange-600 text-white font-black uppercase text-[10px] lg:text-xs tracking-widest hover:from-amber-600 hover:to-orange-700 transition-all shadow-xl shadow-amber-500/20 disabled:opacity-50 flex items-center justify-center gap-2 lg:gap-3">
          {submitPayment.isPending ? (
            <><Loader2 className="w-3.5 h-3.5 lg:w-4 lg:h-4 animate-spin" /> Submitting...</>
          ) : (
            <><ArrowRight className="w-3.5 h-3.5 lg:w-4 lg:h-4" /> Submit Payment</>
          )}
        </button>
      </motion.form>
      ...
      {payments && payments.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <h3 className="text-sm lg:text-lg font-black tracking-tight mb-3 lg:mb-6">Your Payments</h3>
          <div className="space-y-2 lg:space-y-3">
            {payments.map((p: any, idx: number) => (
              <motion.div key={p._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}
                className="p-3 lg:p-5 rounded-xl lg:rounded-2xl bg-white/[0.02] border border-white/5 flex items-center gap-3 lg:gap-4">
                <div className={`w-9 h-9 lg:w-12 lg:h-12 rounded-lg lg:rounded-xl flex items-center justify-center flex-shrink-0 ${p.status === 'approved' ? 'bg-success/10' : p.status === 'rejected' ? 'bg-red-500/10' : 'bg-amber-500/10'}`}>
                  {p.status === 'approved' ? <CheckCircle2 className="w-4 h-4 lg:w-6 lg:h-6 text-success" /> :
                   p.status === 'rejected' ? <X className="w-4 h-4 lg:w-6 lg:h-6 text-red-500" /> :
                   <Clock className="w-4 h-4 lg:w-6 lg:h-6 text-amber-500" />}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] lg:text-xs text-text-muted mt-0.5 truncate">{p.transactionId} • {new Date(p.paymentDate).toLocaleDateString()} • {p.paymentMethod}</p>
                  {p.status === 'rejected' && p.rejectionReason && (
                    <p className="text-[10px] lg:text-xs text-red-400 mt-0.5">Reason: {p.rejectionReason}</p>
                  )}
                </div>
              </motion.div>
            ))}
