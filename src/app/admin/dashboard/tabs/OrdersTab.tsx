...
export default function OrdersTab() {
  ...
  const filteredOrders =
    orders?.filter((o: any) => filter === "All" || o.status === filter) || [];
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-white/5 border-b border-white/5">
                <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[3px] text-text-muted">
                  Customer / ID
                </th>
                <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[3px] text-text-muted">
                  Product / Plan
                ...
                <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[3px] text-text-muted text-right">
                  Actions
                </th>
              </tr>
            </thead>
              ...
              {filteredOrders.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-8 py-20 text-center">
                    ...
                    <p className="text-xl font-black text-text-muted">
                      No orders found
                    </p>
                  </td>
                </tr>
                  ...
                  >
                    ...
                    <td className="px-8 py-8">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-mono text-sm font-black text-text-primary uppercase tracking-widest">
                          {order.transactionId}
                        </span>
                      </div>
                      <p className="text-[10px] text-text-muted font-bold mt-1 uppercase tracking-widest">
                        via {order.paymentNumber}
                      </p>
                    </td>
                    <td className="px-8 py-8">
                      <span
                        className={`px-4 py-1.5 rounded-xl border text-[10px] font-black uppercase tracking-[2px] ${getStatusColor(order.status)}`}
                      >
                        {order.status}
                      </span>
                    </td>
